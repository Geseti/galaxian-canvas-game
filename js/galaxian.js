		...
		function Game() {
			...
			this.start = function() {
				this.ship.draw();
				console.log('ship should be drawn' && this.ship.x);
				this.backgroundAudio.play();
				animate();
			};
		}
		...
		function checkReadyState() {
			if (game.gameOverAudio.readyState === 4 && game.backgroundAudio.readyState === 4) {
				...
				console.log('readystate: audio loaded');
				game.start();
			}
		}
			...
			this.move = function() {
				...
				if (KEY_STATUS.left || KEY_STATUS.right ||
					...
					if (KEY_STATUS.left) {
						...
						this.y -= this.speed
						if (this.y <= this.canvasHeight/4*3)				//Move limitation (boxed in to a grid at the bottom)
